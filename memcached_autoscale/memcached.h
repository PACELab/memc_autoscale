...
struct settings {
    ...
    int timedump_interval;       /* Interval of items to dump timedetails */
};
...
int update_ts(char * key, int len, int ts, conn * c);
int do_update_ts(char * key, int len, int ts, uint32_t hv, conn * c);
enum store_item_type do_local_store_item(item *item, uint32_t hv);
...
int * get_active_slabs(int * count);
...
void append_stat(const char *name, ADD_STAT add_stats, conn *c,
                 const char *fmt, ...);
enum store_item_type local_store_item(item *item);

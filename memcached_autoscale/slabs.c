...
void get_items(unsigned int id, FILE *fp) {
    slabclass_t *p;
    // int i, j, k, found;
    int i, j;
    char *curpage;
    ...
    for (i = 0; i < p->slabs; i++) {
        curpage = p->slab_list[i];
        for (j = 0; j < p->perslab; j++) {
            it = (item *)curpage;
	    /*fr_it = (item *)p->slots;
	    found = 0;
	    for (k = 0; k < p->sl_curr; k++) {
		if (fr_it == it) {
		    found = 1;
		    break;
		}
...
int get_scores(double * weights, int * active_slabs, unsigned int * items_count)
{
    int i;
    int ind = 0;
    ...
    for(i = POWER_SMALLEST; i <= power_largest; i++) {
        ...
        if (p->slabs != 0) {
            uint32_t slabs, perslab, active_items, mem;
            slabs = p->slabs;
	    perslab = p->perslab;
	    active_items = slabs*perslab - p->sl_curr;
	    mem = active_items*p->size;
	    total_slabs += mem;
	    weights[ind] = mem;
	    items_count[ind] = active_items;
	    active_slabs[ind] = i;
            ind++;
        }
    }
    ...
    for (i = 0; i < ind; i++)
    {
        weights[i] /= total_slabs;
        //len = snprintf(temp, sizeof(temp), "%f\r\n", weights[i]);
        //memcpy(buf + bufcurr, temp_ret, len);
        //bufcurr += len;
    }
    pthread_mutex_unlock(&slabs_lock);
    return ind;

...
static rel_time_t srvr_time = 25; //Default

static inline void setSrvr_time(unsigned int time) {
	srvr_time=time;
}
...
int do_update_ts(char * key, int len, int ts, uint32_t hv, conn * c) {
    item * it = do_item_get(key, len, hv, c);
    if (it != NULL) {
	it->time = ts;
	//printf("ts updated\n");
	return	0;
...
enum store_item_type do_local_store_item(item *it, const uint32_t hv) {
    do_item_link(it, hv);
    ITEM_set_cas(it, 0);
    return STORED;
}
...
static void server_stats(ADD_STAT add_stats, conn *c) {
    ...
    APPEND_STAT("SyncTime", "%ld", srvr_time);
...
static void process_stat_settings(ADD_STAT add_stats, void *c) {
    ...
    APPEND_STAT("lru_crawler_timedump_interval", "%d", settings.timedump_interval);
}

static char * find_range_ts(conn * c, int * bytes_ret) {
   unsigned int memlimit = 1024 * 1024;
    char *buffer_ret;
    int bufcurr = 0;
    int len;
    FILE *fp_ts;
    char str[11000];
    char temp[200];
    unsigned long min_ts = 4294967295;
    ...
    while (fgets(str, 10999, fp_ts)) {
        sscanf(str, "%lu", &cur_ts);
        if (cur_ts < min_ts)
	    min_ts = cur_ts;
	if (cur_ts > max_ts)
	    max_ts = cur_ts;
    }
    len = snprintf(temp, sizeof(temp), "max:%lu\n", max_ts);
    memcpy(buffer_ret + bufcurr, temp, len);
    bufcurr += len;
    len = snprintf(temp, sizeof(temp), "min:%lu\n", min_ts);
    memcpy(buffer_ret + bufcurr, temp, len);
    bufcurr += len;
    fclose(fp_ts);
...
static char * warmup(unsigned long min_ts, int to_add, conn * c, int * bytes_ret) {
    unsigned int memlimit = 1024 * 1024;
    char *buffer_ret;
    int bufcurr = 0;
    int len;
    ...
    printf("minimum ts :%lu\n", min_ts);
    unsigned long last_ts = max_ts;
    while (fgets(str, 10999, fp)) {
	xx = fgets(str_ts, 10999, fp_ts);
	if (xx == 0)
	    printf("failed reading ts line\n");
	sscanf(str_ts, "%lu", &cur_ts);
	//printf("curr ts read +++: %lu\n", cur_ts);
	cur_ts = cur_ts - min_ts;
	//printf("curr ts: %lu\n", cur_ts);
	if (max_ts < cur_ts)
	    max_ts = cur_ts;
	if (last_ts > cur_ts)
	    max_failed += 1;
	last_ts = cur_ts;
        sscanf(str,"%s %d %s", k, &vlen, val);
        vlen += 2;
        it = item_alloc(k, nkey, 0, realtime(0), vlen);
        if (it == NULL) {
            printf("alloc failed\n");
            failed_cnt++;
        }
        else {
            memmove(ITEM_data(it), val, it->nbytes);
            //store_item(it, NREAD_SET, c);
	    local_store_item(it);
	    it->time = cur_ts;
            item_remove(it);
            insert_cnt++;
	    //update_ts(k, 11, cur_ts, c);
        }
    }
    len = snprintf(temp, sizeof(temp), "%lu\n", max_ts);
    memcpy(buffer_ret + bufcurr, temp, len);
    bufcurr += len;
    add_to_time = to_add+10;
    fclose(fp);
    fclose(fp_ts);
    /*
    len = snprintf(temp, sizeof(temp), "total max updated failed : %d\n", max_failed);
    memcpy(buffer_ret + bufcurr, temp, len);
    bufcurr += len;
    len = snprintf(temp, sizeof(temp), "total inserted : %d\n", insert_cnt);
    memcpy(buffer_ret + bufcurr, temp, len);
    bufcurr += len;
    len = snprintf(temp, sizeof(temp), "total failed : %d\n", failed_cnt);
    memcpy(buffer_ret + bufcurr, temp, len);
    bufcurr += len;*/
    memcpy(buffer_ret + bufcurr, "END\r\n", 6);
    bufcurr += 5;
...
static char * do_migrate_sets(conn *c, int myid, int * retiring_servers, int rts_count, int * bytes_ret) {
    ...
    //int slab_count = 0;
    //printf("get slabs\n");
    //slabs = get_active_slabs(&slab_count);
    //printf("slabs are here \n");
    //printf("slabs count : %d\n", slab_count);

    buffer_ret = malloc((size_t)memlimit);
    printf("allocated buffer\n");

    //for (ii = 0; ii < slab_count; ii++)
    //{
	//i = slabs[ii];
	//printf("doing slab %d\n", i);
        //if (1==1)
        //{
            ...
            {
		printf("my id : %d", myid);
                snprintf(temp, sizeof(temp), "/home/ubuntu/mc%d_mc%d.dt", myid, retiring_servers[j]);
		printf("file : %s\n", temp);
                fp = fopen(temp, "r");
                if (!fp)
		{
		    len = snprintf(temp_ret, sizeof(temp_ret), "not found: %d %d\r\n", i, retiring_servers[j]);
                    memcpy(buffer_ret + bufcurr, temp_ret, len);
                    bufcurr += len;
                    continue;
		}

                len = snprintf(temp_ret, sizeof(temp_ret), "%d %d\r\n", i, retiring_servers[j]);
                memcpy(buffer_ret + bufcurr, temp_ret, len);
                bufcurr += len;
		// line_cnt = 0;
                while (fgets(str, 10999, fp)) {
                    ...
                    //printf("key : %s\n", k);
                    vlen += 2;
                    // it = item_get(k, nkey, c);
                    if (!it)
                        it = item_alloc(k, nkey, flags, realtime(exptime), vlen);
                    if (it == NULL)
                    {
                        printf("\niteration : %d, value len : %d\n", i, vlen);
                        // enum store_item_type status;
                        if (! item_size_ok(nkey, flags, vlen)) {
                                printf("SERVER_ERROR object too large for cache\n");
                        } else {
                                printf("SERVER_ERROR out of memory storing object\n");
                        }
...
static void process_sync(conn *c, token_t *tokens, const size_t ntokens) {
    unsigned int new_time;
    if(!safe_strtoul(tokens[SUBCOMMAND_TOKEN].value, &new_time)) {
        out_string(c, "Unable to Synchronize new time");
	return;
    }
    setSrvr_time(new_time); 
}
...
static void process_stat(conn *c, token_t *tokens, const size_t ntokens) {
	...
	double time_taken = ((double)t)/CLOCKS_PER_SEC; // in seconds
        printf("dump() took %f seconds to execute \n", time_taken);
        printf("done \n");
        write_and_free(c, buf, bytes);
	return;
    } else if (strcmp(subcommand, "update_time") == 0) {
	printf("current time : %lu\n", (unsigned long)current_time);
	add_to_time = atoi(tokens[2].value);
	printf("add to time updated %d\n", add_to_time);
	printf("current time : %lu\n", (unsigned long)current_time);
    } else if (strcmp(subcommand, "warmup") == 0) {
        char * buf;
        int bytes, to_add;
	unsigned long min_ts;
        clock_t t;
        t = clock();
	to_add = atoi(tokens[2].value);
	min_ts = strtoul(tokens[3].value,NULL,10);
//	min_ts = strtoul(tokens[3].value);
        buf = warmup(min_ts, to_add, c, &bytes);
        t = clock() - t;
	double time_taken = ((double)t)/CLOCKS_PER_SEC; // in seconds
        printf("warmup() took %f seconds to execute \n", time_taken);
        printf("done \n");
        write_and_free(c, buf, bytes);
	return;
    } else if (strcmp(subcommand, "range") == 0) {
	char  * buf;
	int bytes;
	buf = find_range_ts(c, &bytes);
	write_and_free(c, buf, bytes);
	return;
	...
	clock_t t;
	t = clock();
        myid = atoi(tokens[2].value);
        count = atoi(tokens[3].value);
        int servers[count];

        for (i = 0; i < count; i++)
        {
            servers[i] = atoi(tokens[4+i].value);
            printf("server : %d\n", servers[i]);
        }

        // clock_t t;
        // t = clock();
        buf2 = do_migrate_sets(c, myid, servers, count, &bytes);
        t = clock() - t;
	write_and_free(c, buf2, bytes);
        double time_taken = ((double)t)/CLOCKS_PER_SEC; // in seconds
        printf("migrate() took %f seconds to execute \n", time_taken);
        printf("done \n");
//        write_and_free(c, buf2, bytes);
        return ;
    } else if(strcmp(subcommand, "score") == 0) {
	char * buf;
	int bytes, init, ret;
	init = atoi(tokens[2].value);
	ret = atoi(tokens[3].value);
	buf = scores_all(&bytes, init, ret);
	printf("buf size : %d", bytes);
	write_and_free(c, buf, bytes);
	return;
    } else if(strcmp(subcommand, "update_ts") == 0) {
	char * key;
	int len, ts, ret;
	key = tokens[2].value;
	len = atoi(tokens[3].value);
	ts = atoi(tokens[4].value);
	ret = update_ts(key, len, ts, c);
	printf("out from update ts : %d\n", ret);
    }  else if(strcmp(subcommand, "merge") == 0) {
        //char *buf;
        char *buf2;
        int bytes, count, i, myid;
        myid = atoi(tokens[2].value);
        count = atoi(tokens[3].value);
        int * servers = malloc(count * sizeof(int));
        for (i = 0; i < count; i++) {
            servers[i] = atoi(tokens[4+i].value);
            printf("server : %d\n", servers[i]);
        }
        clock_t t;
        t = clock();
        buf2 = do_merge_all(myid, &bytes, servers, count);
        //t = clock() - t;
        //double time_taken = ((double)t)/CLOCKS_PER_SEC; // in seconds
        //printf("merging() took %f seconds to execute \n", time_taken);
        printf("done \n");
	free(servers);
        write_and_free(c, buf2, bytes);
	t = clock() - t;
	...
	printf("merging() took %f seconds to execute \n", time_taken);
        return ;
    } else if(strcmp(subcommand, "dump_slabs") == 0) {
	dump_slabs();
    } else if(strcmp(subcommand, "dump_slabs_pages") == 0) {
	//unsigned int i;
	//i = atoi(tokens[2].value);
	dump_slabs_pages();
    } else if(strcmp(subcommand, "hash") == 0) {
        char *buf;
        char *buf2;
        int bytes, self, count, i;
	int * servers;
        buf = tokens[2].value;
	self = atoi(tokens[3].value);
	count= atoi(tokens[4].value);
	servers = malloc(count * sizeof(int));
	for (i = 0; i < count; i++) {
		servers[i] = atoi(tokens[5+i].value);
	}
        clock_t t;
        t = clock();
	// dump_slabs();
        buf2 = do_hash_all(buf, &bytes, self, count, servers);
        t = clock() - t;
        double time_taken = ((double)t)/CLOCKS_PER_SEC; // in seconds
        ...
        write_and_free(c, buf2, bytes);
        return ;
    } else if (strcmp(subcommand, "get_slabs") == 0) {
	char * buf;
	int bytes;
	buf = get_slabs(&bytes);
	write_and_free(c, buf, bytes);
	return;
        ...
        write_and_free(c, buf, bytes);
        return ;
    }  else if (strcmp(subcommand, "cachedump_time") == 0) {
        char *buf;
        unsigned int bytes, id, limit = 0;
        ...
        buf = item_cachedump_time(id, limit, &bytes);
        write_and_free(c, buf, bytes);
        return ;
...
/*
static void do_migrate_sets(conn *c, int myid, int * retiring_servers, int rts_count, int slab) {
    FILE *fp;
    char str[11000];
    char k[12];
    char val[10010];
    ...
    item *it=NULL;
    int slab_count = 0;
    int * slabs = get_active_slabs(&slab_count);
	...
	i = slabs[ii];
        if (1==1)
        {
            for (j = 0; j < rts_count; j++)
            {
                snprintf(temp, sizeof(temp), "/home/ubuntu/mc%d_mc%d_slbn=%d.dt_trunc", retiring_servers[j], myid,  i);
                fp = fopen(temp, "r");
                if (!fp)
                    continue;
...
static inline void process_get_command(conn *c, token_t *tokens, size_t ntokens, bool return_cas) {
    ...
    //dump = fopen("/home/ubuntu/hash_dump", "a");
    ...
    do {
		...
		//ketama_continuum con;
          	//ketama_roll(&con, "ketama.servers");
	    //int k_len = nkey;
	    //strncpy(key_temp, key, nkey);
	    //key_temp[nkey] = 0x00; /* terminate */
	    //printf("nkey : %d\n", k_len);
	    //printf("key  : %s\n", key_temp);
	    //mcs * m = ketama_get_server(key_temp, con);
	    //char * server = m->ip;
	    //if (server == NULL)	 {
	     //   printf("fucked up server\n");
	    //}
...
static void process_command(conn *c, char *command) {
    ...
    if (ntokens >= 3 &&
    ...
    } else if (ntokens >= 2 && (strcmp(tokens[COMMAND_TOKEN].value, "sync") == 0)) {

        process_sync(c, tokens, ntokens);
        ...
        */
        ...
        if (ntokens == 4 && strcmp(tokens[COMMAND_TOKEN + 1].value, "crawl") == 0) {
            ...
            return;
        } else if (strcmp(tokens[COMMAND_TOKEN + 1].value, "timedump") == 0) {
            if (settings.lru_crawler == false) {
                out_string(c, "CLIENT_ERROR lru crawler disabled");
                return;
            }
	    printf("inside process command\n");
	    int ret_count, i;
	    int * retServers;
	    printf("server id : %s\n", tokens[3].value);
	    ret_count = atoi(tokens[4].value);
	    printf("ret count : %d\n", ret_count);
	    retServers = malloc((sizeof(int))*ret_count);
	    for (i = 0; i < ret_count; i++) {
		retServers[i] = atoi(tokens[5+i].value);
		printf("ret server id : %d\n", retServers[i]);
	    }
	    preprocess_hashing(tokens[3].value, ret_count, retServers, atoi(tokens[5+ret_count].value));
	    printf("started crawling \n");
	    ...
	    //out_string(c, "OKIEDOKIE");
	    //conn_set_state(c, conn_watch);
            //        event_del(&c->event);
	    free(retServers);
            return;
        } else if (ntokens == 4 && strcmp(tokens[COMMAND_TOKEN + 1].value, "timedump_interval") == 0) {
            uint32_t interval;
             if (!safe_strtoul(tokens[2].value, &interval)) {
                out_string(c, "CLIENT_ERROR bad command line format");
                return;
            }
            settings.timedump_interval = interval;
            out_string(c, "OK");
            return;
...
static void clock_handler(const int fd, const short which, void *arg) {
    ...
    if (monotonic) {
        ...
        if (clock_gettime(CLOCK_MONOTONIC, &ts) == -1)
            return;
        current_time = (rel_time_t) (ts.tv_sec - monotonic_start + add_to_time);
        return;
    }
#endif
    {
        ...
        current_time = (rel_time_t) (tv.tv_sec - process_started + add_to_time);
    }
}

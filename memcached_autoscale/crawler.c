...
//int cr_cnt;
// const void * last_comp1; // add globals ubaid
...
static void crawler_timedump_eval(crawler_module_t *cm, item *search, uint32_t hv, int i);
static void crawler_timedump_finalize(crawler_module_t *cm);
static int crawler_timedump_init(crawler_module_t *cm, void *data);
crawler_module_reg_t crawler_metadump_mod = {
    .init = crawler_timedump_init,
    .eval = crawler_timedump_eval,
    .doneclass = NULL,
    .finalize = crawler_timedump_finalize,
    .needs_lock = false,
    .needs_client = true
};
...
static int crawler_timedump_init(crawler_module_t *cm, void *data) {
    ...
    */
    ...
    all_servers = fopen("ketama1.servers", "r");

    while (fgets(temp, 100, all_servers)) {
        if (strlen(temp) < 3)
            break;
...
static void crawler_timedump_finalize(crawler_module_t *cm) {
    ...
    for (i = 0; i < 10; i++) {
        for (j = 0; j < 256; j++) {
            if (fp[i][j]) fclose(fp[i][j]);
	    if (fp_data[i][j]) fclose(fp_data[i][j]);
        }
    }

    printf("crawling finalize started\n");
  /*  for (i = 0; i < 256; i++) {
	if (slab_files[i]) fclose(slab_files[i]);
    }
*/
    memcpy(cm->c.cbuf, "TIMEDUMPEND\r\n", 6+8);
    bipbuf_push(cm->c.buf, 6+8);
...
//nsk Added Code for AutoScaling
static void crawler_timedump_eval(crawler_module_t *cm, item *it, uint32_t hv, int i) {
    //printf("inside timedump_eval\n");
    //FILE *fp1;
    //char fname[200];
    char val[10500];
    char key_temp[15];
    ...
    // if(items_crawled%(settings.timedump_interval)!=0) {
    ...
    */
    ...
    //snprintf(fname, sizeof(fname), "/home/ubuntu/ts/%u", it->slabs_clsid);
    //fp1 = fopen(fname, "a");
    strncpy(val, ITEM_data(it), it->nbytes-2);
    val[it->nbytes-2] = 0x00;
    strncpy(key_temp, ITEM_key(it), it->nkey);
    key_temp[it->nkey] = 0x00;

    mcs *m = ketama_get_server(key_temp, con);
    file_index = atoi((m->ip)+2)-1;
    if (get_exp() > 0 && (file_index+1) != get_server())
        it->exptime = current_time + 180;
    snprintf(temp, sizeof(temp), "%s %lu %s\n", key_temp, strlen(val), val);
    fputs(temp, fp_data[file_index][it->slabs_clsid]);
    snprintf(temp, sizeof(temp), "%lu\n", (unsigned long)it->time + process_started);
    fputs(temp, fp[file_index][it->slabs_clsid]);

/*
    snprintf(temp, sizeof(temp), "%lu %s %s\n", (unsigned long)(it->time + process_started), key_temp, val);
    fputs(temp, slab_files[it->slabs_clsid]);
*/
    //cr_cnt++;
    //printf("crawled count : %d\n", cr_cnt);
    //fclose(fp1);
   // int total = snprintf(cm->c.cbuf, 4096,
     //       "%llu slb: %u\n",
       //     (unsigned long long)it->time + process_started, it->slabs_clsid);
    /*
    int total = snprintf(cm->c.cbuf, 4096,
            "key=%s  time=%llu \n",
            keybuf,
